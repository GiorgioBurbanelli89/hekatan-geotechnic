...
export class SlopePlot {
  ...
  draw(o: PlotOptions): { lv: number[]; vmin: number; vmax: number } {
    ...
    const mL = 62, mR = 118, mT = 40, mB = 48;
    ...
    for (let z = zt0; z <= grid.zmax + 3; z += 5) { const [, q] = tf(0, z); ctx.fillText(String(z), x0 - 6, q); ctx.beginPath(); ctx.moveTo(x0, q); ctx.lineTo(x0 + 4, q); ctx.stroke(); }
    ctx.textAlign = "center"; ctx.textBaseline = "bottom"; ctx.fillText("x [m]", x0 + pw / 2, H - 4);
    ctx.save(); ctx.translate(14, y0 + ph / 2); ctx.rotate(-Math.PI / 2); ctx.textBaseline = "top"; ctx.fillText("z [m]", 0, 0); ctx.restore();
    ctx.font = "bold 13px Segoe UI, Arial"; ctx.textBaseline = "bottom"; ctx.fillText(o.title, x0 + pw / 2, mT - 8);
    // barra de color (mínimo abajo, como la GUI de GEO5 muestra su rango)
    if (flat) return { lv, vmin, vmax };
    const bx = x0 + pw + 22, by = y0, bh = ph, bw = 18, nb = lv.length - 1;
    for (let b = 0; b < nb; b++) {
